# Fix adminRoutes.js - use correct column names from old schema
Write-Host "Fixing adminRoutes.js column names..." -ForegroundColor Yellow
...
router.get('/users', async (req, res) => {
  try {
    ...
    const verifiedCol = hasVerified ? ', is_verified' : '';
    
    let sql = `SELECT id, name, phone, role, location${verifiedCol}, created_at FROM users`;
    const params = [];
...
router.get('/listings', async (req, res) => {
  try {
    const r = await db.query(
      'SELECT l.id, l.product, l.quantity, l.location, l.availability, l.status, l.created_at, u.name as farmer_name, u.phone as farmer_phone FROM listings l JOIN users u ON l.farmer_id=u.id ORDER BY l.created_at DESC LIMIT 100'
    );
    res.json(r.rows);
  } catch(e) {
